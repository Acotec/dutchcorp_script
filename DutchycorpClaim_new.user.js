            ...
            try {
                ...
                if (_available_link <= 1000) {
                    if (DontOpen_LinkByName(linkName)) {
                        duration = 0;
                        DEBUG&&console.log('Won\'t open', linkName, limit);
                    } else {
                        ...
                        if (shortlinks_name.includes(linkName.replace(/\s/ig, '').toLowerCase())) {
                            ...
                            var Error, count = 0;
                            var checkcount = (count = 0) => { if (count < 0) { count++; } };
